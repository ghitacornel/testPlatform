...
public class CompanyRouteUnregister extends RouteBuilder {
    ...
    public void configure() {
        from("timer://simpleTimer?period=10000&delay=10000")
            ...
            .filter(body()
            .method("size")
            .isGreaterThan(CompanyRouteUnregister.MINIMUM)).setBody(exchange -> GenerateUtils.random(exchange.getMessage().getBody(List.class), random)).choice()
            .when(body().isNull())
                .log(LoggingLevel.WARN, "No companies available for unregistering")
            .otherwise()
                .process(exchange -> {
                    try {
